...
const Orderform = () => {
  ...
  const handleKeyDown = (event, index, onAddProduct) => {
    if (event.key === "Enter") {
      ...
      // const quantityValue = event.target.form[`products.${index}.scale`].value;
      ...
      if (inputs.length === 0) {
        return; // Exit the function if there are no input fields
      }

      const currentIndex = inputs.indexOf(event.target);

      // Check if the current input is the scale textbox
      if (
        currentIndex !== -1 &&
        inputs[currentIndex].name === `products.${index}.scale`
      ) {
        ...
        if (!nameValue) {
          remove(index);
          // Focus on the input field with name "bill_details.bill_amount"
        ...
        // Find the index of the next row's first input field
        ...
        // If a next row exists, focus on its first input field
        if (foundNextRow && inputs[nextIndex]) {
          inputs[nextIndex].focus();
          inputs[nextIndex].select(); // Select text in the next input field
        } else {
          // If there is no next row and nameValue is not null, call onAddProduct function
          if (nameValue) {
            onAddProduct();
          }
        }
